  ...
  /******************* 开启时钟 和 复用IO口*******************/
   ...
   case FTM3_BASE:
    ...
    /******************** 配置时钟和分频 ********************/
		...
		{
    	FTM_SC_REG(ftmn)    = ( 0
                                  | FTM_SC_PS(4)      //分频2^FTM_SC_PS,频率为 40M/16=2.5M
		...
		{
    	FTM_SC_REG(ftmn)    = ( 0
                                  | FTM_SC_PS(3)      //分频2^FTM_SC_PS,频率为 40M/4=10M
                                  | FTM_SC_CLKS(1)    //时钟选择，bus时钟
...
void FTM0_interrupt(void)
{
    ...
    if( s & (1 << CHn) )						//边沿已经产生
    {
			//FTM_IRQ_DIS(FTM1,3);     //禁止输入捕捉中断
			if(GPIO_Get(PTE4))						//上升沿
			{
